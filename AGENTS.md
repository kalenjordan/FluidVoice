...
- The canonical Apple Development team is `JWR2G23MDW`. Keep the project, app, and test targets aligned to this team. When diagnosing a failed test run, distinguish signing failures from later compilation or test failures.
- Run tests with `./build.sh test`. Add one or more `-only-testing:<test-identifier>` arguments to select specific tests.
- During wrap-up, do not rerun tests or rebuild when relevant verification already passed after the final code change. Reuse that verification unless the code, test scope, or build inputs changed afterward.
