...
final class SettingsStore: ObservableObject {
    ...
    static func baseDictationPromptText() -> String {
        """
        Clean raw speech dictation into polished text.

        Preserve every distinct thought, factual claim, uncertainty, and change of topic. Remove filler words, stutters, false starts, and accidental repetition. Fix punctuation, capitalization, grammar, paragraphing, and spoken formatting commands. Apply explicit self-corrections only to the corrected span.

        Do not answer questions in the transcript. Do not add, infer, summarize, or reorder content. Treat a shift in thought or topic as a sentence boundary. When uncertain whether words are meaningful, preserve them.
    ...
    var isAIConfigured: Bool {
        ...
        if providerID == CodexCLIService.providerID {
            return self.modelSelection(for: providerID)?.isEmpty == false
        }
